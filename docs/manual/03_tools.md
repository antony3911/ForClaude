...
## 3.8 Python 端：PyTorch、HuggingFace、ESMFold
...
1. LUT 為什麼可以實現「任何」小型邏輯函數？
2. 什麼是 II（Initiation Interval）？v0 的累加迴圈為什麼做不到 II = 1？
3. v2 為什麼一定要對 `la`、`lb`、`acc` 做 `ARRAY_PARTITION`？
4. C 模擬（csim）通過，代表什麼？不代表什麼？
5. `.xo`、`.xclbin`、platform 分別是什麼？

**下一章**一步一步回顧我們實際做了什麼：每一步為什麼做、程式碼怎麼寫、產生了哪些檔案。
